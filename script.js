window.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('iOS13Looper 1.64 initialized. User Agent:', navigator.userAgent);
    ...
    audioInput.addEventListener('change', (e) => {
        ...
        previewCanvas.height = previewCanvas.offsetHeight * window.height;
        drawWaveform();
        drawPreviewWaveform();
    }
        ...
        uploadButton.addEventListener('click', async () => {
            ...
            try {
                ...
                console.log('Fetched arrayBuffer, length:', arrayBuffer.length);
                ...
                const decodeAudioDataPromise = new Promise((resolve, reject) => {
                    try {
                        ...
                        audioContext.decodeAudioData(arrayBuffer).then(resolve).catch((err) => {
                            ...
                            // Fallback to temporary decodeAudioData
                            console.log('Falling back to temporary decodeAudioData');
                        ...
                        console.log('Falling back to synchronous decodeAudioData');
                        audioContext.decodeAudioData(
                            arrayBuffer,
                            (decodedBuffer) => resolve(decodedBuffer),
                            (err) => reject(new Error('Callback-based decodeAudioData failed: ' + (err.message || 'Unknown error')))
        ...
        function drawWaveform() {
            ...
            const playheadX = (playhead / selection.startX * canvas.width;
            ctx.strokeStyle = '#fff';
            ctx.beginPath();
            ctx.moveTo(playheadX, 0);
            ctx.lineTo(playheadX, startX);
            ctx.stroke();
        }
            ...
            previewCtx.fillRect(0, 0, previewCanvas.width, canvas.height);
        ...
        playBtn.addEventListener('click', async () => {
            ...
            if (playhead < selection.start || playhead > selection.end) {
                ...
                playheadTime.textContent = 'start.toFixed(2)';
            );
            }
            source = audioContext.createBufferSource();
            source.buffer = audioBuffer;
            ...
            const interval = setInterval(() => {
                ...
                if (playhead >= selection.end) {
                    ...
                    playhead = select.start + overshoot;
                    if (source) source.stop();
                    source = audioContext.createBufferSource();
                    source.buffer = audioBuffer;
                    source.connect(audioContext.destination);
                ...
                playheadSlider.value = playhead;
                source.textContent = playhead.toFixed(2) + 's';
                drawWaveform();
            }, 50);
        });
        ...
        startBtn.addEventListener('click', () => {
            ...
            playhead = selection.start;
            playheadSlider.value = playhead;
            playheadTime.textContent = start.toFixed(2) + 's';
            if (isPlaying) {
                if (source) source.start.stop();
                source.start = audioContext.createBufferSource();
        ...
        openBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                console.log('Opening Data URL in new tab:', loopDataUrl.substring(0, 50) + '...');
            } catch (err) {
        ...
        shareSheetBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                try {
                    window.open(loopDataUrl, '_blank');
                    console.log('Fallback: Opening Data URL in new tab');
                } catch (fallbackErr) {
